 <template>
  <div>
    ...
    <p>{{ random_rows }}</p>
    <p>{{ round_num }}</p>
    <button @click="pick_two_random_rows(new_data,3);ask_name_question(new_data,3); what_type_of_question($event); randomly_order_answers($event);gameplay($event)" >START</button>
...
export default {
  ...
  methods: {
    ...
    pick_two_random_rows:function(new_data,chosen_row){
    ...
    gameplay:function(){
      ...
      if(question_type =="Name"){
          this.result=ask_name_question(new_data,random_row)
      }else if(question_type=="Country"){
          this.result=ask_country_question(new_data,random_row)
      }else if(question_type=="Main Achievement"){
          this.result=ask_main_achievement_question(new_data,random_row)
      }else if(question_type=="Key Word"){
          this.result=ask_key_word_question(new_data,random_row)
      }else if(question_type=="Year"){
          this.result=ask_year_question(new_data,random_row)
      }else if(question_type=="Event in Japan"){
          this.result=ask_event_in_japan_question(new_data,random_row)
      }else{
          this.result,"result",ask_period_question(new_data,random_row)
      }
